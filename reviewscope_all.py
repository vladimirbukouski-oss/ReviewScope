...
def summarize_product(summary_obj: Dict[str, Any], reviews: List[Dict[str, Any]], llm_cfg: LLMProviderConfig,
    ...
    system = (
        ...
        "  \"score\": {\"value\": float, \"scale\": 5, \"note\": str},  // Объективная оценка с пояснением\n"
        "  \"pros\": [{\"point\": str, \"evidence_ids\": [str]}],  // 3-6 реальных плюсов из отзывов\n"
        "  \"cons\": [{\"point\": str, \"evidence_ids\": [str]}],  // 2-5 реальных минусов из отзывов\n"
        "  \"aspects\": [{\"aspect\": str, \"summary\": str, \"mentions\": int, \"evidence_ids\": [str]}],\n"
        "  \"fit_size\": {\"summary\": str, \"evidence_ids\": [str]} | null,  // Про размер/посадку если релевантно\n"
        "  \"quality_issues\": [{\"issue\": str, \"evidence_ids\": [str]}],  // Проблемы с качеством если есть\n"
        "  \"who_it_is_for\": [str],  // Конкретные категории людей (2-4 пункта)\n"
        "  \"who_should_avoid\": [str],  // Кому НЕ подойдёт (1-3 пункта)\n"
        "  \"confidence\": {\"level\": \"low\"|\"medium\"|\"high\", \"why\": str},\n"
        "  \"notes\": {\"suspicious_share\": float|null, \"truth_stars\": float|null, \"data\": {...}}\n"
        "}\n\n"
        "ПРАВИЛА НАПИСАНИЯ:\n"
        "1. PROS (плюсы) — пиши конкретно и ёмко:\n"
        "   - НЕ \"Хорошее качество\" → ДА \"Плотная ткань, не просвечивает\"\n"
        "   - НЕ \"Красивый\" → ДА \"Стильный дизайн, смотрится дороже цены\"\n"
        "   - Используй факты из отзывов: материал, посадка, износостойкость\n\n"
        "2. CONS (минусы) — будь честен, но конструктивен:\n"
        "   - НЕ \"Плохое качество\" → ДА \"Швы расходятся после 2-3 стирок\"\n"
        "   - НЕ \"Не понравилось\" → ДА \"Маломерит на размер, берите больше\"\n"
        "   - Указывай частоту проблемы если понятна из отзывов\n\n"
        "3. WHO_IT_IS_FOR — конкретные люди:\n"
        "   - НЕ \"Для всех\" → ДА \"Для офиса и повседневной носки\"\n"
        "   - НЕ \"Кто хочет купить\" → ДА \"Для тех, кто ищет базовую вещь без наворотов\"\n\n"
        "4. WHO_SHOULD_AVOID — честно предупреди:\n"
        "   - \"Тем, кто предпочитает оверсайз — посадка приталенная\"\n"
        "   - \"Если нужна вещь на долгие годы — качество среднее\"\n\n"
        "5. ONE_LINER — главный вывод:\n"
        "   - Формат: \"[Что] + [главное качество] + [вердикт]\"\n"
        "   - Пример: \"Базовая футболка с хорошей посадкой — берите смело\"\n"
        "   - Пример: \"Стильные кроссовки, но узковаты — примерьте перед покупкой\"\n\n"
        "6. ОБЩЕЕ:\n"
        "   - Каждый пункт в pros/cons/quality_issues/aspects ДОЛЖЕН иметь 2-6 evidence_ids\n"
        "   - score.value привязывай к truth_stars из метаданных\n"
        "   - Не придумывай факты, которых нет в отзывах\n"
        "   - Пиши на русском, живым языком, без канцелярита\n"
    )
